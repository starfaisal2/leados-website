...
// Bumped whenever the public Terms & Conditions change. Sent with the intent so
// the CRM records exactly which terms the customer accepted.
...
// Canonical industry values — MUST match the CRM's signup-industries allow-list.
// The form renders the friendly label but POSTs the canonical `value`.
...
export default function GetStartedPage() {
  ...
  const submit = async (event: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      // Step 1 — record the signup intent (no tenant created yet).
      ...
      // Step 2 — create the Stripe Checkout session for the one-time setup fee.
      ...
      <div className="container signup-container">
        <div className="signup-copy">
          ...
          <h1 className="display-lg">
            Set up your LeadOS workspace on the{" "}
            <span className="text-serif-em" style={{ color: "var(--blue)" }}>{selectedPlanLabel}</span> plan.
          </h1>
          <p className="body-lg" style={{ marginTop: 18 }}>
            Complete your details and a one-time setup fee, and our team begins your professional
            onboarding right away.
          </p>
          <div className="signup-trust-list">
            <div><span>✓</span> Professional onboarding included</div>
            <div><span>✓</span> First month included after setup</div>
            <div><span>✓</span> Human + AI setup support</div>
            <div><span>✓</span> Terms &amp; Conditions apply</div>
          </div>
        </div>
        ...
        <form className="signup-card" onSubmit={submit}>
          ...
          <div className="signup-grid-two">
            <label>
              Phone
              <input value={form.phone} onChange={(e) => update("phone", e.target.value)} placeholder="+971 56 835 0424" autoComplete="tel" />
            </label>
            <label>
              Country
              <input value={form.country} onChange={(e) => update("country", e.target.value)} placeholder="UAE" autoComplete="country-name" />
            </label>
          </div>
          ...
          <label style={{ flexDirection: "row", alignItems: "flex-start", gap: 10, fontWeight: 600, cursor: "pointer" }}>
            ...
            <span style={{ fontSize: 13, lineHeight: 1.5, color: "var(--ink-3)" }}>
              ...
              <Link href="/privacy">Privacy Policy</Link>, including the one-time setup fee.
            </span>
          </label>
